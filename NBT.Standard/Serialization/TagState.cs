    ...
    internal sealed class TagState
    {
            ...
            if (type == TagType.List || type == TagType.Compound)
            {
                ...
                else if (_mode == FileAccess.Write && state.ChildCount != state.ExpectedCount)
                {
                    throw new InvalidOperationException(
                        $"Expected {state.ExpectedCount} children, but {state.ChildCount} were written.");
                }
            }
        }
        ...
        public TagContainerState StartTag(TagType type)
        {
            ...
            if (_openTags.Count != 0)
            {
                ...
                if (currentState.ContainerType == TagType.List && currentState.ChildType != TagType.End &&
                    type != currentState.ChildType)
                {
                    throw new InvalidOperationException(
                        $"Attempted to add tag of type '{type}' to container that only accepts '{currentState.ChildType}'.");
                }
